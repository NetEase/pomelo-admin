...
MasterAgent.prototype.listen = function(port) {
  ...
  this.server.sockets.on('connection', function(socket) {
    var id, type, info, registered, username, password;
    ...
    socket.on('register', function(msg, pid) {
      ...
      if (msg && msg.type) {
        if (msg.type === 'client') {
          ...
          // var user = utils.checkAndGetUser(adminUser, username, password, msg.md5);
          ...
          if (msg.id) {
            ...
            var record = addConnection(self, msg.id, msg.type, msg.pid, msg.info, socket);
            id = msg.id;
            type = msg.type;
            info = msg.info;
            registered = true;
      ...
      if(msg && msg.type) {
        if(msg.id) {
          ...
          if(self.idMap[msg.id]) {
            ...
            socket.emit('reconnect_ok', {code: protocol.PRO_FAIL, msg: 'id has been registered. id:' + msg.id});
            return;
          }
          ...
          registered = true;
          msg.info.pid = pid;
          socket.emit('reconnect_ok', {code: protocol.PRO_OK, msg: 'ok'});
