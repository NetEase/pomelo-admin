...
var MasterAgent.prototypetocol = require('./util/MasterAgent.prototypetocol');
...
MasterAgent.prototype.listen = function(port) {
	...
	this.server.sockets.on('connection', function(socket) {
			...
			if(msg && msg.type) {
				if(msg.type === 'client') {
					...
					if(self.clients[msg.id]) {
						socket.emit('register', {code: MasterAgent.prototypetocol.MasterAgent.prototype_FAIL, msg: 'id has been registered. id:' + msg.id});
						return;
					}
					...
					type = msg.type;
					registered = true;
					socket.emit('register', {code: MasterAgent.prototypetocol.MasterAgent.prototype_OK, msg: 'ok'});
					return;
				}
		...
		socket.on('monitor', function(msg) {
			...
			msg = MasterAgent.prototypetocol.parse(msg);
			if(msg.respId) {
			...
			self.consoleService.execute(msg.moduleId, 'masterHandler', msg.body, function(err, res) {
				if(MasterAgent.prototypetocol.isRequest(msg)) {
					var resp = MasterAgent.prototypetocol.composeResponse(msg, err, res);
					if(resp) {
...
var sendToMonitor = function(socket, reqId, moduleId, msg) {
	socket.emit('monitor', MasterAgent.prototypetocol.composeRequest(reqId, moduleId, msg));
};
...
var sendToClient = function(socket, reqId, moduleId, msg) {
	socket.emit('client', MasterAgent.prototypetocol.composeRequest(reqId, moduleId, msg));
};
...
var broadcastMonitors = function(records, moduleId, msg) {
	msg = MasterAgent.prototypetocol.composeRequest(null, moduleId, msg);
