...
MonitorAgent.prototype.connect = function(port, host, cb) {
  ...
  this.socket.on('error', function(err) {
    logger.error('monitor socket error, serverId: %s,  process id: %s, err: %j', self.id, process.pid, err.stack);
  ...
  this.socket.on('disconnect', function(reason) {
    logger.error('monitor disconnect, serverId: %s,  process id: %s, reason: %s', self.id, process.pid, reason);
    self.state = ST_CLOSED;
    self.emit('close');
  });

  this.socket.on('reconnect', function() {
    logger.error('monitor reconnect successfully, serverId: %s,  process id: %s', self.id, process.pid);
    self.state = ST_CONNECTED;
    var req = {
      id: self.id,
      type: 'monitor',
  ...
  this.socket.on('reconnecting', function(delay, times) {
    logger.error('monitor is reconnecting, serverId: %s, delay: %s, reconnect times: %s', self.id, delay, times);
  });
